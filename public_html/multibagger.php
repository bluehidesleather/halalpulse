...
$summary = $app->multibagger->summary();
$companies = $app->multibagger->companies();

usort($companies, static function (array $left, array $right): int {
    $priority = static function (array $company): array {
        $shariaPassed = ($company['sharia_status'] ?? null) === 'passed';
        $scored = ($company['score_status'] ?? null) === 'scored' && $company['final_score'] !== null;
        $score = $scored ? (int) $company['final_score'] : 99;
        $alertEligible = (int) ($company['alert_eligible'] ?? 0) === 1;

        $stage = match (true) {
            $shariaPassed && $scored && $alertEligible => 0,
            $shariaPassed && $scored => 1,
            $shariaPassed => 2,
            default => 3,
        };
...
<div class="page-heading">
    <div><p class="eyebrow">Separate research layer</p><h1>Best research candidates</h1><p class="muted">Sharia-passed companies are prioritized first, then ordered from score 1 strongest to score 10 weakest. The score is a research aid, not a return prediction.</p></div>
</div>
<?php Page::flash($app->session->consumeFlash()); ?>
<?php if ($methodology === null): ?>
<section class="notice-card notice-error policy-gate"><strong>Scoring is locked</strong><p>No reviewed methodology is active. Complete the ignored local methodology file, define evidence requirements and grade anchors for every factor, run <span class="mono">cron/check-multibagger-methodology.php</span>, and activate only a file reported as ready.</p></section>
<?php else: ?>
...
<section class="notice-card"><strong>Methodology discipline</strong><p>Every factor must retain official evidence requirements and explicit grade anchors for 1, 4, 7, and 10. DCF assumptions, market-cap bands, and microcap adjustments are part of the methodology hash and cannot be changed silently.</p></section>
<section class="panel panel-results"><div class="panel-heading"><div><p class="eyebrow">Ranked research queue</p><h2>Strongest eligible companies first</h2></div><span class="status"><?= Page::escape(count($companies)) ?> shown</span></div>
